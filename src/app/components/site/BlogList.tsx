...
export async function CategoryChips({ lang, active }: { lang: string; active?: string }) {
  ...
  return (
    <nav className={s.filters} aria-label="Blog categories">
      ...
      {c.categories.map((cat) => (
        <Link key={cat.key} href={categoryHref(lang, cat.slug)} aria-current={active === cat.slug ? "page" : undefined}>
          {cat.label}
        </Link>
      ))}
    </nav>
...
export async function FeaturedPost({ lang, post }: { lang: string; post: Post }) {
  const c = await getContent(lang);
...
export function PostGrid({ lang, posts }: { lang: string; posts: Post[] }) {
  return (
    <div className="grid-3">
      {posts.map((p, i) => (
        <PostCard key={p.key} lang={lang} post={p} index={i % 3} />
      ))}
    </div>
  );
}
